	...
	Antigravity: {
		caps: Capabilities{
			...
			Notes: []string{
				...
				"--conversation appends to the same conversation transcript rather than forking (observed 1.1.4; envelope conversation_id stability re-confirmed 1.2.7)",
	...
	ClaudeCode: {
		caps: Capabilities{
			...
			Notes: []string{
				...
				"resume preserves the session_id (observed 2.1.212; re-confirmed 2.1.267); chaining refs from each turn's envelope stays correct either way",
	...
	Codex: {
		caps: Capabilities{
			...
			Notes: []string{
				...
				"exec resume appends to the same rollout file and preserves the session id (observed 0.144.6; re-confirmed 0.155.1); a separate exec fork subcommand (present by 0.154.0) forks into a new session instead, but subcommands are not reachable via ExtraArgs, so Resume always means append here",
