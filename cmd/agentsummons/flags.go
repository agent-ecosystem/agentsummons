...
func addRequestFlags(cmd *cobra.Command, f *requestFlags) {
	...
	cmd.Flags().StringVar(&f.sessionID, "session-id", "", "preset the session identity (claude-code and copilot only)")
	cmd.Flags().StringVar(&f.resume, "resume", "", "continue an existing session by explicit ref (see info per harness)")
	cmd.Flags().StringSliceVar(&f.allowedTools, "allowed-tools", nil, "restrict the agent to these tools (claude-code and copilot only)")
