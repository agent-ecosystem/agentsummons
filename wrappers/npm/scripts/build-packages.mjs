...
//
//   node build-packages.mjs --version 0.1.1 --assets <dir> --out <dir>
//
...
for (const p of PLATFORMS) {
  const archive = path.join(assets, `agentsummons_${version}_${p.artifact}.${p.ext}`);
  if (!fs.existsSync(archive)) {
    console.error(`build-packages: missing release archive ${archive}`);
    process.exit(66);
  }
  const pkgDir = path.join(out, "platform", `agentsummons-${p.key}`);
  const binDir = path.join(pkgDir, "bin");
  fs.mkdirSync(binDir, { recursive: true });
  const exe = p.os === "win32" ? "agentsummons.exe" : "agentsummons";
  if (p.ext === "zip") {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: `agentsummons-${p.key}`,
        version,
        description: `agentsummons binary for ${p.key}. Install the "agentsummons" package instead.`,
        os: [p.os],
        cpu: [p.cpu],
        files: ["bin"],
  ...
  fs.writeFileSync(
    ...
    `# agentsummons-${p.key}\n\nPlatform binary for [agentsummons](https://www.npmjs.com/package/agentsummons); installed automatically as an optional dependency. Install \`agentsummons\` itself, not this package.\n`,
  );
...
const mainDir = path.join(out, "agentsummons");
